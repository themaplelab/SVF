...
void LevelByLevelFlowSensitiveStat::clearStat()
{
    _NumOfNullPtr = 0;
    _NumOfConstantPtr = 0;
...
void LevelByLevelFlowSensitiveStat::statNullPtr()
{
    _NumOfNullPtr = 0;
    for (SVFIR::iterator iter = levpa->getPAG()->begin(), eiter = levpa->getPAG()->end();
            iter != eiter; ++iter)
    {
        ...
        if (inComingStore.empty()==false || outGoingLoad.empty()==false)
        {
            ...
            const PointsTo& pts = levpa->getPts(pagNodeId);
            if(levpa->containBlackHoleNode(pts))
            {
                _NumOfConstantPtr++;
            }
            if(levpa->containConstantNode(pts))
            {
                _NumOfBlackholePtr++;
            }
            if(pts.empty())
...
void LevelByLevelFlowSensitiveStat::statPtsSize()
{
    // TODO: needs to be made to work for persistent.
    if (SVFUtil::isa<LevelByLevelFlowSensitive::MutDFPTDataTy>(levpa->getPTDataTy()))
    {
        // stat of IN set
        statInOutPtsSize(levpa->getDFInputMap(), IN);
        // stat of OUT set
        statInOutPtsSize(levpa->getDFOutputMap(), OUT);
    }
            ...
            iter != eiter; ++iter)
    {
        ...
        if (levpa->getPAG()->isValidTopLevelPtr(iter->second) == false)
            continue;
        u32_t size = levpa->getPts(node).count();
...
void LevelByLevelFlowSensitiveStat::statInOutPtsSize(const DFInOutMap& data, ENUM_INOUT inOrOut)
{
    ...
    for (; it != eit; ++it)
    {
        const SVFGNode* node = levpa->svfg->getSVFGNode(it->first);
    ...
    for (SVFG::const_iterator it = svfg->begin(); it != svfg->end(); ++it)
    {
        ...
        if (const StoreSVFGNode *store = SVFUtil::dyn_cast<StoreSVFGNode>(sn))
        {
            ...
            for (NodeID o : levpa->ander->getPts(p)) incomingObjects.set(o);
 ...
 * Points-to size
 */
void LevelByLevelFlowSensitiveStat::statAddrVarPtsSize()
{
    SVFG::SVFGNodeIDToNodeMapTy::const_iterator it = levpa->svfg->begin();
    SVFG::SVFGNodeIDToNodeMapTy::const_iterator eit = levpa->svfg->end();
    for (; it != eit; ++it)
    {
 ...
 * Points-to size
 */
void LevelByLevelFlowSensitiveStat::calculateAddrVarPts(NodeID pointer, const SVFGNode* svfg_node)
{
    const PointsTo& pts = levpa->getPts(pointer);
    ...
    for (; ptsIt != ptsEit; ++ptsIt)
    {
        ...
        const PointsTo& cpts = levpa->getDFOutPtsSet(svfg_node, ptd);
...
void LevelByLevelFlowSensitiveStat::performStat(){
    assert(SVFUtil::isa<LevelByLevelFlowSensitive>(levpa) && "not an flow-sensitive pta pass!! what else??");
    endClk();

    clearStat();

    SVFIR* pag = levpa->getPAG();
    ...
    SVFG::iterator svfgNodeIt = levpa->svfg->begin();
    SVFG::iterator svfgNodeEit = levpa->svfg->end();
    ...
    timeStatMap["SolveTime"] = levpa->solveTime;
    timeStatMap["SCCTime"] = levpa->sccTime;
    timeStatMap["ProcessTime"] = levpa->processTime;
    timeStatMap["PropagationTime"] = levpa->propagationTime;
    timeStatMap["DirectPropaTime"] = levpa->directPropaTime;
    timeStatMap["IndirectPropaTime"] = levpa->indirectPropaTime;
    timeStatMap["Strong/WeakUpdTime"] = levpa->updateTime;
    timeStatMap["AddrTime"] = levpa->addrTime;
    timeStatMap["CopyTime"] = levpa->copyTime;
    timeStatMap["GepTime"] = levpa->gepTime;
    timeStatMap["LoadTime"] = levpa->loadTime;
    timeStatMap["StoreTime"] = levpa->storeTime;
    timeStatMap["UpdateCGTime"] = levpa->updateCallGraphTime;
    timeStatMap["PhiTime"] = levpa->phiTime;
    ...
    PTNumStatMap["SolveIterations"] = levpa->numOfIteration;

    PTNumStatMap["IndEdgeSolved"] = levpa->getNumOfResolvedIndCallEdge();
    ...
    PTNumStatMap["StrongUpdates"] = levpa->svfgHasSU.count();
    ...
    PTNumStatMap["ProcessedAddr"] = levpa->numOfProcessedAddr;
    PTNumStatMap["ProcessedCopy"] = levpa->numOfProcessedCopy;
    PTNumStatMap["ProcessedGep"] = levpa->numOfProcessedGep;
    PTNumStatMap["ProcessedLoad"] = levpa->numOfProcessedLoad;
    PTNumStatMap["ProcessedStore"] = levpa->numOfProcessedStore;
    PTNumStatMap["ProcessedPhi"] = levpa->numOfProcessedPhi;
    PTNumStatMap["ProcessedAParam"] = levpa->numOfProcessedActualParam;
    PTNumStatMap["ProcessedFRet"] = levpa->numOfProcessedFormalRet;
    PTNumStatMap["ProcessedMSSANode"] = levpa->numOfProcessedMSSANode;

    PTNumStatMap["NumOfNodesInSCC"] = levpa->numOfNodesInSCC;
    PTNumStatMap["MaxSCCSize"] = levpa->maxSCCSize;
    PTNumStatMap["NumOfSCC"] = levpa->numOfSCC;
    timeStatMap["AverageSCCSize"] = (levpa->numOfSCC == 0) ? 0 :
                                    ((double)levpa->numOfNodesInSCC / levpa->numOfSCC);
